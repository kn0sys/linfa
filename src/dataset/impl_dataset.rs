...
impl<F, E, I: TargetDim> Dataset<F, E, I> {
    ...
    pub fn split_with_ratio(mut self, ratio: f32) -> (Self, Self) {
        ...
        let mut array_buf = self.records.into_raw_vec_and_offset().0;
        ...
        let mut array_buf = self.targets.into_raw_vec_and_offset().0;
        ...
        let second_weights = if self.weights.len() == n1 + n2 {
            let mut weights = self.weights.into_raw_vec_and_offset().0;
